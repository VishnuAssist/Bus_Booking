...
export function ValidateParams(fromparams: QueryParamsType) {
  ...
  if (fromparams?.endDate) params["endDate"] = fromparams?.endDate?.toString();
  if (fromparams?.Category)
    params["category"] = fromparams?.Category?.toString();
  if (fromparams?.Status) params["Status"] = fromparams?.Status?.toString();
  if (fromparams?.Ids) params["Ids"] = fromparams?.Ids?.toString();
  return params;
}
